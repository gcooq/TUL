...
#2_GRU of TUL model 
#Gowalla Data
#We delete the Chinese annotation
...
import os
#parameter sets
learning_rate=0.00095 #0.00095
train_iters=30 #iteration number
dispaly_step=1000
batch_size=1 #
User_List=list() #
...
def RNN(x,weights,biases,keep_prob): #Define RNN
    ...
    n_splits=n_steps[0]
    lstm_cell_1=tf.nn.rnn_cell.GRUCell(n_hidden) #原始遗忘门0.3
    lstm_cell_2 = tf.nn.rnn_cell.GRUCell(n_hidden)
...
pred,traceVector=RNN(x,weights,biases,keep_prob)

cost=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(pred,y_out))

optimizer=tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost) #
#evaluate model
correct_pred=tf.equal(tf.arg_max(pred,1),tf.argmax(y_out,1)) #dim
accuracy=tf.reduce_mean(tf.cast(correct_pred,tf.float32))

# 
init=tf.initialize_all_variables()
def getXs(): #read POI embedding
    fpointvec=open('data/gowalla_vector_new250d.dat','r') #get check-in tensor while word2vec
    ...
    return table_X
def printXs():
    fuservec_out=open('out_data/gowalla-vector_out.dat','w')#output files
    out_table=getXs()
...
def getUvector(i):
    return table_Y[i]
def compare(sess,test_T,test_U,allAcc,User_List,iter,filename='out_data/TEST_resultVec.txt'):  #
    ...
    for i in range(len(tempU)):
        ...
        Dic[i]=[0,0,0,0] # use mask for record results
    #print COU_LIST
    #print Dic
    while step<len(test_T): #
        ...
        xsx_step=[map(lambda x:getPvector(x), [a for a in test_T[step]])] # convert ID to tensor 

        xsy_step=[getOneHot(get_mask_index(test_U[step],User_List))] #get user onehot
        user_id=get_mask_index(test_U[step],User_List)
        #print user_id
        Dic.get(user_id)[2]+=1 #a+c 
        
        nowVec=sess.run(pred,feed_dict={x:xsx_step,y_out:xsy_step,keep_prob:1.0})[0] #
       ...
       # print test_U[step]
        ...
        #if(index<5):
            #AccTop5+=1
        predictPred=sess.run(correct_pred,feed_dict={x:xsx_step,y_out:xsy_step,keep_prob:1.0})
        if(predictPred==1):
            acc+=1
...
def start(n_steps):
    ...
    with tf.Session() as sess:
        ...
        tempT,pointT,userT,seqlens,test_T,test_UserT,test_lens,User_List=readtraindata() #
...
#                 print "xsy_step",xsy_step
                ...
                sess.run(optimizer,feed_dict={x:xsx_step,y_out:xsy_step,keep_prob:0.5})
#                 print "ACC",sess.run(accuracy,feed_dict={x:xsx_step,y_out:xsy_step})
                if(sess.run(correct_pred,feed_dict={x:xsx_step,y_out:xsy_step,keep_prob:0.5})==1):
                    acc+=1
#                 now=sess.run(pred,feed_dict={x:xsx_step,y_out:xsy_step})[0]
#                 print "now",now,len(now)
                if(step%dispaly_step==0):
                    loss=sess.run(cost,feed_dict={x: xsx_step,y_out:xsy_step,keep_prob:0.5})
        ...
        for i in range(n_hidden): #print W
            for j in range(n_classes):
                fresultw.write('%f '% rweights[i][j])
            fresultw.write('\n')       
        
        for i in range(n_classes): #print bias
